...
# Predefined answers for common suggested questions
CANNED_RESPONSES = {
    "What is your current research focus?": (
        "Berardino’s current research focuses on probabilistic and causal machine learning for medical AI, "
        "with an emphasis on vision language models, longitudinal disease modeling, and counterfactual analysis. "
        "At McGill University and the Mila AI Institute in Montréal, he develops methods for trajectory prediction, "
        "treatment effect estimation, and biomarker discovery using large scale 3D medical imaging and multimodal data. "
        "His work integrates deep generative models, neural SDEs, and LLM based zero shot reasoning to support "
        "robust decision making in clinical trials and disease management."
    ),

    "What experience do you have with vision language models?": (
        "Berardino has extensive experience with vision language models, including the design of probabilistic "
        "active learning frameworks for CLIP style biomedical VLMs. His work introduces stochastic prompts, "
        "probabilistic adapters, and cross modal alignment to improve uncertainty estimation and label efficiency "
        "in low data medical settings. He also leverages LLMs for contextualized class descriptions, zero shot "
        "prediction, and causal reasoning over longitudinal image and tabular embeddings."
    ),

    "What projects have you worked on in medical AI?": (
        "Berardino has worked on multiple large scale medical AI projects, primarily in neuroimaging and "
        "neurodegenerative diseases. These include multiple sclerosis disease progression modeling using over "
        "60,000 3D MRI scans from more than 10,000 patients, lesion segmentation with U-Net architectures, "
        "brain connectivity analysis via graph and tensor factorization methods, and counterfactual modeling "
        "of grey matter atrophy and treatment effects. His projects span classification, prognosis, and "
        "clinical trial evaluation."
    ),

    "What are your strongest machine learning skills?": (
        "Berardino’s strongest skills include deep learning for computer vision, probabilistic modeling, "
        "and causal inference. He has deep expertise in PyTorch and TensorFlow, CNNs, generative models, "
        "longitudinal modeling with LSTMs and neural SDEs, uncertainty quantification, and end to end ML pipelines. "
        "He is also highly proficient in statistical learning, model evaluation, and deploying scalable training "
        "workflows using Docker and AWS."
    ),

    "What experience do you have with causal machine learning?": (
        "Berardino has strong experience in causal machine learning across both public policy and healthcare. "
        "He has developed counterfactual and uplift models to estimate treatment and policy effects, using methods "
        "such as potential outcomes, individual treatment effect estimation, and longitudinal causal modeling. "
        "His work includes evaluating drug efficacy in clinical trials, simulating counterfactual disease "
        "trajectories, and assessing policy impacts for governmental and industrial stakeholders."
    ),

    "What experience do you have building end to end ML systems?": (
        "Berardino has extensive experience building end to end machine learning systems from data ingestion "
        "to model training, evaluation, and deployment seamlessly. He has designed scalable training pipelines, "
        "implemented robust validation and monitoring strategies, and packaged models using Docker for cloud "
        "environments such as AWS. His work bridges industrial research and production, ensuring that advanced models are "
        "reliable, reproducible, and suitable for real world decision making."
    ),
}
...
def clear_text():
    ...
    st.session_state.show_canned = False
    st.session_state.selected_suggestion = None
# Chat input layout
if "show_canned" not in st.session_state:
    st.session_state.show_canned = False
...
# Align action buttons with the 3-column suggested questions layout below.
...
# Show a predefined response immediately after clicking a suggested question
if st.session_state.show_canned and st.session_state.selected_suggestion in CANNED_RESPONSES:
    q = st.session_state.selected_suggestion
    a = CANNED_RESPONSES[q]
    st.markdown(f"**You:** {q}")
    st.markdown(
        f"<div style='background-color:#f0f2f6;padding:16px;border-radius:12px;'>{a}</div>",
        unsafe_allow_html=True,
    )

if ask_btn and query.strip():
    # If the question is one of the predefined suggestions, do not call the chatbot
    if query.strip() in CANNED_RESPONSES:
        st.session_state.show_canned = True
        st.session_state.selected_suggestion = query.strip()
        q = st.session_state.selected_suggestion
        a = CANNED_RESPONSES[q]
        st.markdown(f"**You:** {q}")
        st.markdown(
...
st.markdown("**Suggested questions (make sure to click twice):**")

suggested_questions = list(CANNED_RESPONSES.keys())

cols = st.columns(3)
for i, q in enumerate(suggested_questions):
    with cols[i % 3]:
        if q == "What are your strongest machine learning skills?":
